  ...
  for (let i = 0; i < numFlowers; i++) {
    for (let j = 0; j < numFlowers; j++) {
      ...
      if (
        ...
        // 특정한 색상의 꽃을 클릭했을 때만 다른 페이지로 이동
        const clickedFlowerColor = flowerColors[clickedFlowerIndex];
        if (clickedFlowerColor.levels[1] < 80) {
          // 채도가 80 미만인 꽃을 클릭했을 때
          window.location.href = '다른페이지주소'; // 다른 페이지로 이동
        }
